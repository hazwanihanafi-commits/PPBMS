  ...
  async function loadStudent() {
    try {
      const data = await apiGet(
        `/api/admin/student/${encodeURIComponent(email)}`
      );

      setStudent(data.row || null);
      setTimeline(data.row?.timeline || []);
    } catch (err) {
      console.error("Admin load student error:", err.message);
      setStudent(null);
    } finally {
      setLoading(false);
    }
        ...
        <div className="min-h-screen bg-purple-50 p-6 space-y-6">
          ...
          {activeTab === "overview" && (
            <div className="bg-white p-6 rounded-2xl shadow text-gray-700">
              This is a read-only administrative view for monitoring
              student progress, CQI actions, and PLO attainment
              in compliance with MQA requirements.
            </div>
          )}
            ...
            <div className="space-y-6">
              ...
              <div className="bg-white rounded-2xl p-6 shadow">
                <h3 className="font-bold mb-4">
                  📊 CQI by Assessment (Supervisor Input)
                </h3>
