    ...
    <div className="min-h-screen bg-gradient-to-br from-[#dfe9f3] via-[#f5f7fa] to-[#e0c3fc] flex items-center justify-center p-4 relative">
      ...
      <div className="w-full max-w-[1200px] rounded-3xl bg-white/40 backdrop-blur-xl shadow-2xl border border-white/30 overflow-hidden relative z-10">
        ...
        <header className="flex justify-between items-center px-6 py-4 border-b border-white/30 bg-white/30 backdrop-blur-md">
          ...
          <Link href="/login" className="px-5 py-2 rounded-full bg-purple-600 text-white text-sm hover:bg-purple-700 transition">
            Login
          </Link>
        </header>
        ...
        <div className="text-center px-6 py-16">
          ...
          <Link href="/login" className="inline-block px-6 py-3 rounded-full bg-gradient-to-r from-purple-600 to-indigo-500 text-white font-medium shadow-md hover:opacity-90 transition">
            Get Started
          </Link>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 px-6 pb-16">
          <GlassCard title="Student" desc="Track milestones and upload documents." href="/login" />
          <GlassCard title="Supervisor" desc="Monitor supervisees and identify risks." href="/supervisor" />
          <GlassCard title="Admin" desc="Manage CQI reporting and programme analytics." href="/admin/login" />
        </div>
...
function GlassCard({ title, desc, href }) {
  return (
    <div className="p-6 rounded-2xl bg-white/30 backdrop-blur-lg border border-white/40 shadow-md hover:shadow-xl hover:-translate-y-1 transition duration-300">
      ...
      <Link href={href} className="text-purple-600 text-sm font-medium hover:underline">
        Go to login ->
      </Link>
    </div>
  );
