...
import { API_BASE, apiGet, apiPost } from "../../utils/api";
...
export default function StudentPage() {
  ...
  const progress = timeline.length ? Math.round((timeline.filter(t => t.status === "Completed").length / timeline.length) * 100) : 0;
          ...
          <tbody>
            {timeline.map((t, i) => (
              <tr key={i} className="border-t">
                ...
                <td className="p-2">
                  {!t.actual && (
                    <button onClick={() => markCompleted(t.activity)} className="px-3 py-1 bg-green-500 text-white rounded">
