...
export default function SupervisorStudentTable({ students = [], showSupervisorColumn = false, showActionsForAdmin = false }) {
  if (!students || students.length === 0) return <div className="text-sm text-gray-500">No students found.</div>;
      ...
      <tbody>
        {students.map((s) => (
          <tr key={s.id || s.email} className="border-b">
            ...
            <td className="p-2">
              <div className="font-semibold">{s.progressPct ?? s.progress ?? "—"}%</div>
            </td>
            <td className="p-2">{s.lastSubmission ?? "—"}</td>
            <td className="p-2">{s.nextDue ?? "—"}</td>
            <td className="p-2">
              {s.status === "overdue" ? <span className="text-red-600">Overdue</span> : s.status === "warning" ? <span className="text-orange-600">Warning</span> : <span className="text-green-600">OK</span>}
            </td>
            <td className="p-2">
              <div className="flex gap-2">
                ...
                {showActionsForAdmin && <a className="text-indigo-600 hover:underline">Edit</a>}
              </div>
            </td>
          </tr>
        ))}
